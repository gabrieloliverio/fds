...
func createFiles(tempDir, inputContent string, t *testing.T) *os.File {
...
func TestLineReplacer_Replace(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			replacer := NewLineReplacer(tc.search, tc.replace, tc.flags)
			result, _ := replacer.Replace(tc.subject)
...
func TestLineReplacer_ReplaceStringRange(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			replacer := NewLineReplacer(tc.search, tc.replace, tc.flags)
			result := replacer.ReplaceStringRange(tc.subject, tc.stringRange)

			if result != tc.want {
				t.Errorf("ReplaceMatch() = %q, want %q", result, tc.want)
			}
		})
	}
}
